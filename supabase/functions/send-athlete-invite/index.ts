...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (role !== "athlete") {
      return jsonResponse(400, { ok: false, error: "Only athlete invites can be sent by this function" });
    }
    ...
    const [{ data: canWrite }, { data: canManage }] = await Promise.all([
      adminClient.rpc("can_write_team", {
        p_team_id: inviteRow.team_id,
        p_user_id: userData.user.id,
      }),
      adminClient.rpc("can_manage_team_coaches", {
        p_team_id: inviteRow.team_id,
        p_user_id: userData.user.id,
      }),
    ]);
    if (canWrite !== true && canManage !== true) {
    ...
    const [{ data: team, error: teamError }, { data: athlete, error: athleteError }] = await Promise.all([
      adminClient.from("teams").select("name").eq("id", inviteRow.team_id).maybeSingle(),
      adminClient
        .from("team_athletes")
        .select("display_name,first_name,last_name,email")
        .eq("team_id", inviteRow.team_id)
        .eq("id", inviteRow.athlete_profile_id)
        .maybeSingle(),
    ]);
    ...
    console.error("send-athlete-invite failed", error);
